...
public class JsonUtil {
    ...
    public static class JacksonObjectMapper extends ObjectMapper {
        ...
        public JacksonObjectMapper() {
            ...
            super.configure(JsonParser.Feature.ALLOW_NON_NUMERIC_NUMBERS, true);
            //反序列化时，属性不存在的兼容处理s
            super.getDeserializationConfig().withoutFeatures(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
            //日期格式化
            super.registerModule(new SaasJavaTimeModule());
            super.findAndRegisterModules();
            // 反序列化时，使用BigDecimal转换Number类型的属性。防止转为Double时精度丢失
            super.enable(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS);
        }
